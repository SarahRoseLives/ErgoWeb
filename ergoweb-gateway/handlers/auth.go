...
import (
	...
	"ergoweb-gateway/config"
	"ergoweb-gateway/irc"
	"ergoweb-gateway/session"
)
...
// RegisterRequest defines the expected JSON for the registration endpoint.
...
// RegisterHandler handles a new user account registration request.
func RegisterHandler(c *gin.Context) {
	...
	// Construct the saregister endpoint URL from the configured API URL
	ergoRegisterURL := strings.Replace(config.Cfg.ErgoAPIURL, "check_auth", "saregister", 1)

	// Prepare the request body for Ergo's saregister endpoint
	body := make(map[string]string)
	body["accountName"] = req.Username
	body["passphrase"] = req.Password
	if req.Email != "" {
		body["email"] = req.Email
	}
	payload, _ := json.Marshal(body)

	// Create and send the HTTP request to Ergo
	httpReq, _ := http.NewRequest("POST", ergoRegisterURL, bytes.NewReader(payload))
	httpReq.Header.Set("Content-Type", "application/json")
	httpReq.Header.Set("Authorization", "Bearer "+config.Cfg.BearerToken)

	resp, err := http.DefaultClient.Do(httpReq)
	if err != nil {
		log.Printf("Ergo registration request failed: %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{"success": false, "message": "Gateway error during registration"})
		return
	}
	defer resp.Body.Close()
	...
	if err != nil {
		log.Printf("Failed to read Ergo response body: %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{"success": false, "message": "Failed to read registration response"})
		return
	}

	// Parse the JSON response from Ergo
	var result map[string]interface{}
	if err := json.Unmarshal(respData, &result); err != nil {
		log.Printf("Failed to unmarshal Ergo response: %v", err)
	...
	if resp.StatusCode != http.StatusOK || result["success"] != true {
		msg, _ := result["error"].(string) // Per Ergo API docs, the field is "error"
		if msg == "" {
			msg = "Registration failed for an unknown reason."
		}
		log.Printf("Ergo registration failed for user '%s': status %d, message: %s", req.Username, resp.StatusCode, msg)
		// Use 409 Conflict for "account exists" or similar errors
		c.JSON(http.StatusConflict, gin.H{"success": false, "message": msg})
		return
	}

	log.Printf("Successfully registered account '%s' via Ergo", req.Username)
	// Use 201 Created for a successful resource creation
	c.JSON(http.StatusCreated, gin.H{"success": true, "message": "Account registered successfully"})
}
