...
class ApiService {
  ...
  Future<Map<String, dynamic>> register(String username, String password, {String? email}) async {
    ...
    try {
      ...
      print("[ApiService] register: Received status code ${response.statusCode}");
      // The response body should always be decoded, as it contains success or error info.
      final responseData = json.decode(response.body);
      return responseData;
    } catch (e) {
      print("[ApiService] register Error: $e");
      // Return a standardized error map for network issues.
